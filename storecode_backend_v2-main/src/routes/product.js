...
const baseUrlImage= 'http://192.168.0.110:3000';
const upload = multer({
    storage: storage,
...
//obtener tenis
router.get('/storecode/products/tenis',(req,res)=>{
    mysqlConnection.query('CALL psMosTenis()',(err,rows,fields)=>{
        if(!err){
            res.json(rows[0]);
            console.log(rows[0]);
        }else{
...
//obtener sandalias
router.get('/storecode/products/sandalias',(req,res)=>{
    mysqlConnection.query('CALL psMosSandalias()',(err,rows,fields)=>{
        if(!err){
            res.json(rows[0]);
            console.log(rows[0]);
        }else{
...
//obtener botas
router.get('/storecode/products/botas',(req,res)=>{
    mysqlConnection.query('CALL psMosBotas()',(err,rows,fields)=>{
        if(!err){
            res.json(rows[0]);
            console.log(rows[0]);
        }else{
...
//obtener casual
router.get('/storecode/products/casual',(req,res)=>{
    mysqlConnection.query('CALL psMosCasual()',(err,rows,fields)=>{
        if(!err){
            res.json(rows[0]);
            console.log(rows[0]);
        }else{
...
//obtener bebes
router.get('/storecode/products/bebes',(req,res)=>{
    mysqlConnection.query('CALL psMosBebes()',(err,rows,fields)=>{
        if(!err){
            res.json(rows[0]);
            console.log(rows[0]);
        }else{
